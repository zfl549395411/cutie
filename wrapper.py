...
class ReadMemoryONNXWrapper(nn.Module):
    ...
    def forward(self, pix_feat, key, selection, last_mask,
        ...
        # mem_shrinkage = mem_shrinkage.unsqueeze(0)
        # mem_value = mem_value.unsqueeze(0)
...
class SegMentONNXWrapper(nn.Module):
    ...
    def forward(self, f16,f8,f4,readout_memory,sensory, current_ti):
        # f16:              1*1024*H*W  H=h/16 W=w/16
        # f8:               1*512*(h/8)*(w/8)
        # f4:               1*256*(h/4)*(w/4)
        # readout:          1*1*256*H*W  
        ...
        # 取最大通道值用于分割
        mask = torch.argmax(pred_prob_with_bg_, dim=0, keepdim=True).to(torch.float32)
...
def ExportEncoderOnnx(image_height,image_width):
    ...
    onnx_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/image_encoder_480_752.onnx"
    onnx_simplified_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/image_encoder_480_752_sim.onnx"
    # 导出 ONNX，指定输出命名
    torch.onnx.export(
        wrapper,
        dummy_input,
    ...
    print(f"ONNX模型已简化并保存到：{onnx_simplified_path}")
...
def ExportReadMemoryOnnx(image_height,image_width):
    ...
    work_len = H * W * 10
    long_len = 128 * 20
    mem_total_len =  5640 #2480 #work_len + long_len
    ...
    onnx_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/read_memory_480_752.onnx"
    onnx_simplified_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/read_memory_480_752_sim.onnx"
    # 导出 ONNX
    torch.onnx.export(
        wrapper,
        dummy_inputs,
...
def ExportSegmentOnnx(image_height, image_width):
    ...
    onnx_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/segment_480_752.onnx"
    onnx_simplified_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/segment_480_752_sim.onnx"
    torch.onnx.export(
        wrapper,
...
def ExportEncodeMaskOnnx(image_height, image_width):
    ...
    onnx_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/encode_mask_480_752.onnx"
    onnx_simplified_path = "/media/sti/B20F0FD71CF7DE70/cutie/onnx_s100/encode_mask_480_752_sim.onnx"
    # 导出到 onnx
    torch.onnx.export(
        wrapper,
...
def ExportCompressMemOnnx(image_height, image_width):
    ...
    torch.onnx.export(
        ...
        "./onnx_3588/compress_mem_304_480.onnx",
